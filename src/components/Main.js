...
function ModalCenter(props) {
    ...
    const fetchEvolutionData = async (chain) => {
        ...
        const traverseEvolutionChain = async (evolutionChain) => {
            ...
            await Promise.all(evolutionChain.evolves_to.map(async (evolution) => {
        ...
        <Modal {...props} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
            <Modal.Header closeButton>
                <Modal.Title id="contained-modal-title-vcenter">
                    <h3 className="d-flex align-items-center"><img src="https://icon-library.com/images/small-pokeball-icon/small-pokeball-icon-5.jpg" className="icon" />Pokemon #{pokemon.id}</h3>
                </Modal.Title>
            </Modal.Header>
            {dataLoading === true ?  (
                    ...
                    <section>
                        ...
                        <div className="stats">
                            ...
                            {pokemon.stats.map(stats => (
                                <span>
                                    <label>{stats.stat.name} {`${stats.base_stat}%`}</label>
                                    <div className="range">
                                        <div className={`percentage ${stats.stat.name}`} style={{ width: `${stats.base_stat}%` }}></div>
                                    </div>
                                </span>
                            ))}
                        </div>
            ...
            (<div className="main"><Navbar />
                <main>
                    {pokemonList.map(pokemon => (
                        <div className="card" key={pokemon.id} onClick={(e) => getPokemonID(pokemon.id)}>
                            <h4>{pokemon.id}</h4>
                            <figure>
                                <img src={pokemon.sprites["versions"]["generation-v"]["black-white"]["animated"]["front_default"]} alt={pokemon.id} />
                            </figure>
